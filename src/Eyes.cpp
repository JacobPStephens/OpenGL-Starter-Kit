...
Eyes::Eyes(Ghost* parentGhost, float tileSize)
    ...
    :GameObject(glm::vec2(1.0f), glm::vec2(1.0f), tileSize, ResourceManager::GetTexture("eyesLeft"), 1.0f, glm::vec3(1.0f, 1.0f, 1.0f), 1.0f, 0.0f, std::string("eyes"))
{   
    this->parentGhost = parentGhost;
    this->CycleLength = 5.0f;
    this->currSprite = 0;
    this->Size = glm::vec2(14.0f/8.0f * tileSize);
    this->AnimationSprites = GetSprites();
}
